...
for i=1:num_elements     %I verified that ordering of columns is correct!
    An(i,2) = -(n_mob*B_n(1,i+1) + n_mob*B_n(2,i+1+1));     %I HAVE VERIFIED THAT ALL THE dV's properly  match up with  indices! All the extra +1's are b/c B starts at i=2 (1st pt inside device).
end
for i = 1:num_elements-1
    An(i,1) = n_mob*B_n(2,i+1+1);    %should be i+1+1 b/c have B(dVi)*p_i-1  (see fortran reindexed version)
end
An(num_elements, 1) = 0;     %last element is unused
for i = 2:num_elements
    An(i,3) = n_mob*B_n(1,i+1);      %1st element here corresponds to the 1st row of matrix: so need to use B3 --> corresponding to p(3)
end
