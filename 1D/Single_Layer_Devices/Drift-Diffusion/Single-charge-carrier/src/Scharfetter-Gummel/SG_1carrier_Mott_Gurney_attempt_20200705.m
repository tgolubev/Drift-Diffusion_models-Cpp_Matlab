...
num_cell = 1000;            % number of cells    %NOTE: IF WANT TO USE LARGE NUMBER OF POINTS, COMMENT OUT THE CREATION OF ZEROS ARRAYS IN BERNOULLIFNC, OTHERWISE GET ERROR,
%THAT ARRAY SIZE IS TOO  LARGE!
p_initial =  10^27;        %initial hole density   %NOTE: WORKS FOR UP TO 5*10^27 BEYOND THAT, HAVE ISSUES
p_mob = 2.0*10^-4;         %hole mobility
...
%------------------------------------------------------------------------------------------------        
       %% now solve eqn for p  
        ...
        error_p = max(abs(newp-old_p)./abs(old_p))  %MUST USE ./ TO DO ELEMENT BY ELEMENTS: OTHERWISE IT INTERPRETS THIS AS A MATRIX SOLVER CALL!
...
for j = 1:Va_cnt
    Jp_theory(j) = (9*p_mob*epsilon*(V_values(j))^2)/(8*L^3);  % CAN'T seem to match this expression for JV based on Mott-Gurney
    % but matching the E field expression....
end
