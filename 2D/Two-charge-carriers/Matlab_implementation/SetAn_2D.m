...
for index = 1:N*(N-1)      %(1st element corresponds to Nth row  (number of elements = N*(N-1)
    ...
    An_val(index,1) = -((n_mob(i,j) + n_mob(i+1, j))/2.)*Bn_negZ(i+1,j+1);
end
...
%subblocks)
    ...
    if(mod(index,N) == 0)
        ...
        An_val(index,2) = -((n_mob(i,j) + n_mob(i,j+1))/2.)*Bn_negX(i+1,j+1);
    end
end
...
for index =  1:num_elements      
    ...
    An_val(index,3) = ((n_mob(i,j) + n_mob(i,j+1))/2.)*Bn_posX(i+1,j+1) + ((n_mob(i+1,j) + n_mob(i+1,j+1))/2.)*Bn_negX(i+1+1,j+1) + ((n_mob(i,j) + n_mob(i+1,j))/2.)*Bn_posZ(i+1,j+1) + ((n_mob(i,j+1) + n_mob(i+1,j+1))/2.)*Bn_negZ(i+1,j+1+1);
end
...
for index = 2:num_elements     %matlab fills this from the bottom (so i = 2 corresponds to 1st row in matrix)
    ...
    if(i == 0)     %i = 0, corresponds to this--> is actually  i = N
        ...
        An_val(index,4) = -((n_mob(i+1,j) + n_mob(i+1,j+1))/2.)*Bn_posX(i+1+1,j+1);
    end
end
...
for index = 1+N:num_elements      %matlab fills from bottom, so this starts at 1+N (since 1st element is in the 2nd subblock of matrix)
    ...
    An_val(index,5) = -((n_mob(i,j+1) + n_mob(i+1,j+1))/2.)*Bn_posZ(i+1,j+1+1);            %1st element corresponds to 1st row.   this has N^2 -N elements
end
