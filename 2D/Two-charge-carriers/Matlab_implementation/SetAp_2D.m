...
%subblocks)
    ...
    if(mod(index, N) == 0)
        ...
        Ap_val(index,2) = -((p_mob(i,j) + p_mob(i,j+1))/2.)*Bp_posX(i,j);
    end
end
...
for index =  1:num_elements      
    ...
    Ap_val(index,3) = ((p_mob(i,j) + p_mob(i,j+1))/2.)*Bp_negX(i,j) + ((p_mob(i+1,j) + p_mob(i+1,j+1))/2.)*Bp_posX(i+1,j) + ((p_mob(i,j) + p_mob(i+1,j))/2.)*Beclep_negZ(i,j) + ((p_mob(i,j+1) + p_mob(i+1,j+1))/2.)*Bp_posZ(i,j+1);
end
...
for index = 1+N:num_elements      %matlab fills from bottom, so this starts at 1+N (since 1st element is in the 2nd subblock of matrix)
    ...
    Ap_val(index,5) = -((p_mob(i,j+1) + p_mob(i+1,j+1))/2.)*Bp_negZ(i,j+1);            %1st element corresponds to 1st row.   this has N^2 -N elements
end
