        ...
        if(j ==1)  %different for 1st subblock
            for i = 1:N
                ...
                if (i==1)  %1st element has 2 BC's
                    bn(index,1) = Cn*Un(i,j) + n_mob(i,j)*(Bn_negX(i,j)*n_leftBC(1) + Bn_negZ(i,j)*n_bottomBC);  %NOTE: rhs is +Cp*Un, b/c diagonal elements are + here, flipped sign from 1D version                   
                elseif (i==N)
                    bn(index,1) = Cn*Un(i,j) + n_mob(i,j)*(Bn_negZ(i,j)*n_bottomBC + Bn_posX(i+1,j)*n_rightBC(1));
                else
                    bn(index,1) = Cn*Un(i,j) + n_mob(i,j)*Bn_negZ(i,j)*n_bottomBC;
                end
            end
        elseif(j == N)  %different for last subblock
            for i = 1:N
                ...
                elseif (i==N)
                    ...
                    bn(index,1) = Cn*Un(i,j) + n_mob(i,j)*Bn_posZ(i,j+1)*n_topBC;
                end
            end
            ...
            for i = 1:N
                index = index +1;
                if(i==1)
                    bn(index,1) = Cn*Un(i,j) + n_mob(i,j)*Bn_negX(i,j)*n_leftBC(j);
                elseif(i==N)
                    bn(index,1) = Cn*Un(i,j) + n_mob(i,j)*Bn_posX(i+1,j)*n_rightBC(j);
                else
