...
%     p_mob_Z_avg(i+1,j+1,k+1)*Bp_negZ(i+1,j+1,k+1) ...
%                     + p_mob_Y_avg(i+1,j+1,k+1)*Bp_negY(i+1,j+1,k+1) ...
%                     + p_mob_X_avg(i+1,j+1,k+1)*Bp_negX(i+1,j+1,k+1) ...
%                     + p_mob_X_avg(i+1+1,j+1,k+1)*Bp_posX(i+1+1,j+1,k+1) ...
%                     + p_mob_Y_avg(i+1,j+1+1,k+1)*Bp_posY(i+1,j+1+1,k+1) ...
%                     + p_mob_Z_avg(i+1,j+1,k+1+1)*Bp_posZ(i+1,j+1,k+1+1);
...
for index = 1+N*N:num_elements      %matlab fills from bottom, so this starts at 1+N (since 1st element is in the 2nd subblock of matrix) 
    Ap_val(index,7) = values(i+1,j+1,k+1+1);%p_mob_Z_avg(i+1,j+1,k+1+1)*Bp_negZ(i+1,j+1,k+1+1);         
