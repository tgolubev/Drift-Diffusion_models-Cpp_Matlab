%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         3D Drift Diffusion for Electrons and Holes with Finite Differences
%
...
num_elements = N^3;  %NOTE: this will specify number of elements in the solution vector V which = (num_cell +1 -2)^3 b/c we are in 3D
...
%reshape the V vector into the 3D V matrix
...
n = min_dense*ones(num_elements, 1);
p = n;
...
for k = 1:N
    for j = 1:N  %1st vertical subblock set--> the j's iterate
        p_leftBC_x(j, k) = p(index + (j-1)*N + 1);  %just corresponds to values of V in the 1st subblock
        p_rightBC_x(j, k) = p(index + j*N);
        n_leftBC_x(j, k) = n(index + (j-1)*N + 1);  %just corresponds to values of V in the 1st subblock
...
for Va_cnt = 0:num_V +1
    ...
    if(Va_cnt ==1)
        ...
        w=w_i;
        G = zeros(num_elements, 1); %GenerationRate();  %only use it once, since stays constant
    end
    if(Va_cnt >0)
...
%prefactorized LU plus \.
        ...
        if(Va_cnt > 0)
            Up = G;  %NOTE: G is just zeros here! %these only  include insides since want ctoo be consistent with i,j matrix indices
    ...
    if(Va_cnt ==0)
        ...
        for k = 2:num_cell
            i = floor(num_cell/2);
            j = floor(num_cell/2);
            fprintf(equil,'%.2e %.2e %.2e %.8e %.8e %.8e \r\n ',(i-1)*dx, (j-1)*dx, (k-1)*dx, Vt*fullV(i,j,k), N_dos*fullp(i,j,k), N_dos*fulln(i,j,k));
        end
        fclose(equil);
    end
    
    Up_matrix = reshape(Up,N,N,N);
%     Un_matrix = reshape(Un,N,N);
    ...
    if(Va_cnt > 0)
        for k = 2:num_cell
            ...
            j = floor(num_cell/2);
            fprintf(fid,'%.2e %.2e %.2e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e\r\n', (i-1)*dx, (j-1)*dx, (k-1)*dx, Vt*fullV(i,j,k), N_dos*fullp(i,j,k), N_dos*fulln(i,j,k), J_total_X(i,j,k), J_total_Z(i,j,k), Up_matrix(i-1,j-1,k-1), w, tolerance);
        end
    end
    fclose(fid);
