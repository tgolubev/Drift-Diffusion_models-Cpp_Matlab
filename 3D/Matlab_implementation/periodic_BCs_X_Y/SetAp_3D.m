...
Ap_val = zeros(2*num_elements, 7);   %this is a matrix which will just store the non-zero diagonals of 2D hole continuity eqn
...
values_cut = permute(values_cut, [3 2 1]);
Ap_val(1+N*(N+1):num_elements + N*(N+1) ,10) = values_cut(:);  %shifted by N*N+1 b/c N z values and N+1 y values to shift over by.

%--------------------------------------------------------------------------
%far upper diagonal: X right PBC's
values = -p_mob_X_avg.*Bp_negX;
values_cut = zeros(N+1, N+1, N);
values_cut(1, 1:N+1,1:N) = values(N+1+1, 2:N+1+1, 2:N+1); %i = N+1+1 b/c always corresonding to far right BC
values_cut = permute(values_cut, [3 2 1]);
Ap_val(1+(N^2)*(N+1):num_elements + (N^2)*(N+1) ,11) = values_cut(:);  %shifted b/c spdiags fills upper diags from the bottom

%--------------------------------------------------------------------------
Ap = spdiags(Ap_val, [-(N^2)*(N+1) -N*(N+1) -N^2 -N -1 0 1 N N^2 N*(N+1) (N^2)*(N+1)], num_elements, num_elements); %A = spdiags(B,d,m,n) creates an m-by-n sparse matrix by taking the columns of B and placing them along the diagonals specified by d.
